...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Starkcoin";
#else
    fs::path pathRet;
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "starkcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "starkcoind.pid"));
